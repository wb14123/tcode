...
use crate::edit::line_trimmed_replacer::LineTrimmedReplacer;
...
#[test]
fn trimmed_indentation_mismatch() -> anyhow::Result<()> {
    let content = "fn main() {\n    println!(\"hello\");\n}\n";
    // old_string has wrong indentation (no leading spaces)
    let result = LineTrimmedReplacer::replace(
        content,
        "println!(\"hello\");",
        "    println!(\"world\");",
        false,
...
#[test]
fn trimmed_not_found() {
    let content = "fn main() {}\n";
    let result = LineTrimmedReplacer::replace(content, "nonexistent", "replacement", false);
    assert!(result.is_err());
}

#[test]
fn trimmed_multiple_matches_error() {
    let content = "  aaa\nbbb\n  aaa\n";
    let result = LineTrimmedReplacer::replace(content, "aaa", "ccc", false);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("2 times"));
}

#[test]
fn trimmed_replace_all() -> anyhow::Result<()> {
    let content = "  aaa\nbbb\n  aaa\n";
    let result = LineTrimmedReplacer::replace(content, "aaa", "ccc", true)?;
    assert_eq!(result, "ccc\nbbb\nccc\n");
    Ok(())
}

#[test]
fn trimmed_multiline_block() -> anyhow::Result<()> {
    let content = "fn foo() {\n    let x = 1;\n    let y = 2;\n}\n";
    let old = "let x = 1;\nlet y = 2;";
    let new = "    let x = 10;\n    let y = 20;";
    let result = LineTrimmedReplacer::replace(content, old, new, false)?;
