...
///
/// Also simplifies array-form `"type": ["string", "null"]` (generated by
/// schemars 1.x for `Option<T>`) into `"type": "string"`, since many local
/// backends (e.g. llama.cpp Jinja templates) expect `type` to be a plain
/// string. Optionality is already expressed via the `required` array, so
/// dropping `"null"` from `type` is safe for all providers.
pub fn normalize_schema(schema: &Schema) -> serde_json::Value {
    ...
    if let Some(obj) = value.as_object_mut() {
        ...
        // Remove $schema — not expected by LLM tool-calling backends
        obj.remove("$schema");
    }

    simplify_nullable_types(&mut value);
    value
}

/// Recursively walk a JSON value and convert array-form `"type": ["T", "null"]`
/// into `"type": "T"` so that backends expecting a plain string don't break.
fn simplify_nullable_types(value: &mut serde_json::Value) {
    match value {
