...
- **`PermissionDecision`**: AllowOnce, AllowSession, AllowProject, or `Deny { reason: Option<String> }`. When the user supplies a reason, it is sanitized (whitespace collapsed to single spaces) and appended to the raw permission-denied error returned from `ask_permission`, so the same text reaches both the user (via the tool output chunk in the display) and the LLM (inside the `build_tool_result_content` wrapper around the raw content) from a single source — no separate side-channel.
