...
fn send_resolve(socket_path: &PathBuf, key: PermissionKey, decision: PermissionDecision) -> Result<()> {
    let msg = ClientMessage::ResolvePermission { key, decision };
        ...
        if event::poll(std::time::Duration::from_millis(200))? {
            if let Event::Key(key) = event::read()? {
                ...
                if let Some(decision) = decision {
                    ...
                    send_resolve(&args.socket_path, pk, decision)?;
                    return Ok(());
                }
            }
        }
