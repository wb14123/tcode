...
-- collapses the open block before any new element opens. A collapsed block is
-- ONE real buffer row, resolved through the integer row map (no extmarks).
...
test('collapse bulk: content materialized in the full projection then collapsed', function()
  ...
  local block = first_of_type(m, 'thinking_block')
  local l = lines_of(b)
  check(l[1] == 'BULK THINKING' and l[2] == 'more', 'bulk load materialized the thinking content in one shot')
  local r = row_of(m, block)
  check(r.start_row == 0 and r.height == 2, 'row map: open block at [0, 2)')
