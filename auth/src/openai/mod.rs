...
//! the provider's profile-aware `~/.tcode/auth/openai_tokens*.json` files.
...
impl TokenManager {
    ...
    pub fn storage_path(profile: Option<&str>) -> PathBuf {
        crate::oauth_token_storage_path(crate::OAuthProvider::OpenAi, profile)
    }

    #[cfg(test)]
    pub(crate) fn storage_path_in(home_dir: &std::path::Path, profile: Option<&str>) -> PathBuf {
        crate::oauth_token_storage_path_in(home_dir, crate::OAuthProvider::OpenAi, profile)
    }
    ...
    pub fn load(profile: Option<&str>) -> Option<Self> {
        ...
        Self::load_from_file(&path, OpenAiRefresher)
    }
