...
/// Generate a session ID that doesn't collide with existing sessions
/// in `base_dir` or (optionally) `trash_dir`. Retries up to 64 times.
/// Returns the generated ID on success, or an error if all attempts collide.
pub fn generate_unique_session_id(base_dir: &Path, trash_dir: Option<&Path>) -> Result<String> {
    for _ in 0..64 {
        let id = generate_session_id();
        if !base_dir.join(&id).is_dir() && !trash_dir.is_some_and(|t| t.join(&id).is_dir()) {
            return Ok(id);
        }
    }
    bail!("failed to generate a unique session id after 64 attempts")
}
